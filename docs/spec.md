...
**Rework shipped 2026-08-18 (HANDWRITE).** Profiling the BenchRec corpus (D012) showed this formula's assumptions fail on real reconciliation data: account matching contributes zero discrimination on single-account data, ±2-minor-unit amount decay is blind to real fee gaps, and the ±120s time window is meaningless at day granularity. Two figures in the 2026-08-17 draft of this note were wrong and are corrected here by measurement on the full 47,024-pair 1:1 subset: (a) collisions are 12.6% of ledger legs facing more than one candidate, not 29% of statement lines; (b) the shared signal is *not* token-level — whole-token overlap holds for only 25.0% of true pairs, against 82.4% for character 6-grams, because references sit embedded inside longer runs (`V41806706` inside `V418067061617140`). The shipped scorer therefore uses character n-gram overlap, compared by overlap coefficient rather than Jaccard because the two sides differ greatly in length. Threshold moved 0.85 → 0.79: discrimination between candidates is comparative, not absolute within a pair, so the threshold stays permissive enough that a true pair with no text overlap still clears it. Missing text is distinguished from zero text — both sides without grams renormalizes over the remaining weights, one side without grams scores zero and still counts, so blank data can never outrank weak data. Account mismatch is a veto rather than a weighted term. Measured effect on the full corpus: match rate 0.8873 → 0.9334, false-positive rate 0.0996 → 0.0459. Tie-break-to-pending (margin rule) is designed but not yet implemented.
